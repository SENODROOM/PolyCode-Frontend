...
import {
  ...
  POWERSHELL_LESSONS,
  POWERSHELL_TOTAL_XP,
} from "../data/powershellCurriculum";
import usePowerShellProgress from "../hooks/usePowerShellProgress";

const BASE_PATH = "/learn/powershell";
...
export default function PowerShellHub() {
  ...
  const {
    ...
    bookmarks,
    lastLessonId,
  } = usePowerShellProgress();

  const completedCount = Object.keys(progress).length;
  const earnedXP = POWERSHELL_LESSONS.filter((lesson) => progress[lesson.id]).reduce(
    (sum, lesson) => sum + lesson.xp,
    0,
  );
  const pct =
    Math.round((completedCount / POWERSHELL_LESSONS.length) * 100) || 0;
  const nextLesson =
    POWERSHELL_LESSONS.find((lesson) => !progress[lesson.id]) ||
    POWERSHELL_LESSONS[0];
  const resumeLesson =
    POWERSHELL_LESSONS.find((lesson) => lesson.id === lastLessonId) ||
    nextLesson;
  const completedChapters = POWERSHELL_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => POWERSHELL_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return POWERSHELL_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
  ...
  return (
    <div className="oops-hub powershell-hub">
      <div className="oops-hero powershell-hero">
        <Link
        ...
        <div className="oops-hero-badge">POWERSHELL \· AUTOMATION TRACK</div>
        ...
        <p className="oops-hero-sub">
          Master variables, logic, functions, arrays, and objects with friendly
          theory, quizzes, and hands-on PowerShell challenges.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${POWERSHELL_LESSONS.length} lessons · ${earnedXP}/${POWERSHELL_TOTAL_XP} XP`
                  : `Sign in to track progress · ${POWERSHELL_LESSONS.length} lessons`}
              </span>
          ...
          <div className="oops-resume-panel">
            ...
            >
              {completedCount > 0 ? "Resume PowerShell" : "Start PowerShell"}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="oops-guide-tools">
        <div className="oops-tool-panel oops-tool-panel-main">
          <span className="oops-interactive-label">Find a PowerShell topic</span>
          <div className="oops-search-row">
            <input
              ...
              aria-label="Search PowerShell lessons"
            />
            <div
              className="oops-filter-tabs"
              aria-label="Filter PowerShell lessons"
            >
              {[
                ["all", "All"],
                ["todo", "To do"],
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{POWERSHELL_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{POWERSHELL_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{POWERSHELL_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
      ...
      <div className="oops-path-overview">
        {POWERSHELL_CHAPTERS.map((chapter, index) => {
